...
void ClaudioHNSynthesizer::updateFromEngram(float f0, 
    ...
    for (int i = 0; i < maxHarmonics; ++i) {
        if (i < numHarmonics) {
            ...
            // SOTA: Apply Phase Alignment
            if (phaseAlignment > 0.5f && i < phases.size())
                harmonicOscillators[i].resetPhase(phases[i]);
    ...
    this->spectralFlux = spectralFlux; // Cache for the process loop

    float resonanceCutoff = std::min(f0 * 4.0f, 19000.0f);
    noiseFilter.setCutoffFrequency(resonanceCutoff * (1.0f + spectralFlux * 0.2f));
    transientFilter.setCutoffFrequency(12000.0f); 
}
