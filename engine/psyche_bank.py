...
class CogRule(pydantic.BaseModel):
    ...
    def validate_rule_fields(self) -> CogRule:
        ...
        if self.source not in ["tribunal", "manual", "vast_learn"]:
            raise ValueError("CogRule.source must be 'tribunal', 'manual', or 'vast_learn'")
        return self
...
class PsycheBank:
    ...
    async def capture(self, rule: CogRule, ttl_seconds: int | None = None) -> bool:
        ...
        # Prevent auto-capture of rules that block billing or Google domains.
        _BILLING_KEYWORDS = {"pay", "billing", "credit", "purchase", "payment", "google.com"}
        if any(k in validated_rule.pattern.lower() for k in _BILLING_KEYWORDS):
            logger.warning(f"PsycheBank: Rule 4 violation attempt. Rejecting rule '{validated_rule.id}' that targets billing.")
            return False
    ...
    async def purge_expired(self) -> int:
        ...
        with TTLs are automatically cleaned up.
        """
        if not self._initialized.is_set():
    ...
    def _is_valid_rule_id(self, rule_id: str) -> bool:
        ...
        # Check for common sensitive keywords. This is a warning, not a hard block.
        sensitive_keywords = ["password", "secret", "token", "api_key", "credential", "private_key", "certificate", "auth", "key", "user_id", "session"]
        if any(keyword in rule_id.lower() for keyword in sensitive_keywords):
            logger.warning(f"Rule ID '{rule_id}' contains a sensitive keyword. Consider using a more abstract or anonymized ID.")
    ...
    def _is_valid_rule_category(self, category: str) -> bool:
        ...
        allowed_categories = {"security", "quality", "style", "performance", "maintainability"}
    ...
    async def _run_ttl_expiry_checker(self) -> None:
        ...
        while True:
            try:
                ...
                # elapsed their TTL are automatically removed.
