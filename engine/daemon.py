...
class BackgroundDaemon:
    ...
    async def _cycle(self):
        ...
        # Run recursive summary agent on Hot Memory (hourly gate)
        loop = asyncio.get_event_loop()
        now = datetime.now(timezone.utc)
        if not hasattr(self, "_last_distill") or (now - self._last_distill).total_seconds() >= 3600:
            ...
            self._broadcast({
                ...
                "processed": distill_result.get("processed", 0),
                "facts_extracted": distill_result.get("facts_extracted", 0),
            })
