...
def _keyword_overlap(text: str, entry: "BuddyMemoryEntry") -> float:
    ...
    from engine.semantics import jaccard_similarity, tokenize
    
    query_words = set(tokenize(text))
    entry_words = (
        set(tokenize(entry.summary))
        | set(tokenize(entry.last_message_preview))
        | {w.lower() for w in entry.key_topics}
    )
    if not query_words or not entry_words:
        return 0.0
    return jaccard_similarity(query_words, entry_words)
