...
def _jaccard(a: str, b: str) -> float:
    """Jaccard similarity of token sets for two strings (0.0 – 1.0).

    Pure token-overlap — no external dependencies, fully offline, O(|union|).
    """
    ta = frozenset(_normalize(a).split())
    tb = frozenset(_normalize(b).split())
...
class BuddyCache:
    ...
    def _l1_lookup(
        ...
        for entry in reversed(entries[-20:]):
            if entry.intent != intent:
                continue
            score = _jaccard(text, entry.normalized_text)
            if score > best_score:
                best_score = score
                best_entry = entry
    ...
    def _save_knowledge_cache(self) -> None:
        """Atomic write to disk. Errors are silently swallowed — non-fatal."""
        try:
            self._path.parent.mkdir(parents=True, exist_ok=True)
            tmp = self._path.with_suffix(".json.tmp")
            tmp.write_text(
                json.dumps(self._knowledge_cache,
