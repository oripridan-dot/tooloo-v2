...
class TestMCPManager:
    ...
    EXPECTED_TOOLS = {
        ...
        "web_lookup", "run_tests", "run_tests_isolated", "read_error", "spawn_process",
...
class TestNStrokeHappyPath:
    ...
    def test_stroke_record_shape(self) -> None:
        ...
        assert s.stroke == 1
        assert s.satisfied
        # 9 MCP tools (added patch_apply + render_screenshot)
    ...
    def test_mcp_tools_injected_in_execution_metadata(self) -> None:
        ...
        for env in logged_envs:
            ...
            assert len(env.metadata["mcp_tools"]) == 10
...
class TestNStrokeModelEscalation:
    ...
    def _no_throttle(self) -> None:
        """Disable ResourceGovernor throttling so stroke-count tests are deterministic."""
        with patch(
            "engine.resource_governor.ResourceGovernor.get_recommended_strokes",
            side_effect=lambda default_strokes=7: default_strokes,
        ):
            yield
...
class TestNStrokeHTTPEndpoints:
    ...
    def test_n_stroke_mcp_tools_in_stroke_record(self, client: TestClient) -> None:
        ...
        assert len(stroke["mcp_tools_injected"]) == 10
