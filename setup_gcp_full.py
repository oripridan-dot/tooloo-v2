...
def push_github_secret(sa_key_json: str, token: str) -> None:
    ...
    if resp.status_code != 200:
        print(
            f"    [!] Could not fetch repo public key: {resp.status_code} {resp.text}")
        return
    ...
    public_key = nacl_public.PublicKey(
        pub_key.encode(), encoding.Base64Encoder)
    ...
    put_resp = requests.put(secret_url, headers=headers,
    ...
    if put_resp.status_code in (201, 204):
        ...
        print(
            f"    [!] Secret upload failed: {put_resp.status_code} {put_resp.text}")
...
def trigger_workflow(token: str) -> None:
    ...
    resp = requests.post(dispatch_url, headers=headers,
                         json=payload, timeout=15)
    ...
    if resp.status_code == 204:
        ...
        print(f"\n    Manual trigger URL:")
        print(
            f"    https://github.com/{GH_OWNER}/{GH_REPO}/actions/workflows/gcp-crucible.yaml")
